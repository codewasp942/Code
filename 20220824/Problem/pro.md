...
SHW 看了你上一题的答案，觉得补作业太痛苦了，于是他找来了玄学大师 YCY ，希望他帮自己算一卦，以此制定做作业的计划以减轻自己的痛苦。

具体来说， YCY 告诉 SHW ，他在第 $i$ 天因为玄学原因，写作业效率为 $v_i$ （ $ 0 < v_i < 2 $ ），即写一份时间为 $t$ 的作业，实际耗时为 $t \cdot v_i$ 。而且每天写作业的**数量**（不是时间）是有上限的，为 $w_i$ （保证 $\sum_{i=1}^{n}{w_i} \ge m$ ）。这样，通过合理安排作业计划， SHW 就可以减轻自己的痛苦。
...
第一行两个整数 $n,m$ ，含义如上一题所述。

第二行 $m$ 个整数，第 $i$ 个整数表示 $t_i$ 。含义如上一题所述。
...
如果遇到了敌人，会有**一名**兄弟义无反顾地牺牲自己，保护纸条安全，但如果此时身边一名兄弟也没有了，那么纸条就会有一部分被撕掉抢走。
...
接下来 $m$ 行，每行可能有3个或5个整数。前两个整数 $u, v$ 表示一条从 $u$ 到 $v$ 的边。第三个数 $op$ 取值为 $op \in \{0, 1\}$ ，其中0表示这条边上是兄弟；1表示这条边上是敌人，并且接下来两个整数 $a, b$ 表示敌人会抢走纸条后，纸条还剩下 $\frac{a}{b}$ 部分（即乘上 $\frac{a}{b}$ ）（ 保证 $0 < a < b$ ）。

**本题可能有重边。**

## 输出格式

输出一行一个分数，形如 `a/b` 。如果到达不了 CYB ，输出 `BE` 。如果答案为1，输出 `HE` 。  
...
对于另外 $20 \%$ 的数据， $n \le 10^2,m \le 2 \cdot 10^3$ 。

对于 $100 \%$ 的数据， $n \le 10^3,m \le 2 \cdot 10^5$ 。
